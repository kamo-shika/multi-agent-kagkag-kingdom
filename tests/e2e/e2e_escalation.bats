...
@test "E2E-004-A: busy agent defers inbox processing until idle" {
    ...
    send_to_pane "$citizen1_pane" "busy_hold 6"
    sleep 2  # Ensure busy state is active

    # 4. Verify agent is busy (pane shows Working)
    run wait_for_pane_text "$citizen1_pane" "esc to interrupt" 5
    assert_success
...
import yaml
try:
    with open('$E2E_QUEUE/queue/tasks/citizen1.yaml') as f:
...
@test "E2E-004-B: busy_hold shows correct state transitions" {
    ...
    send_to_pane "$citizen1_pane" "busy_hold 4"
