---
name: king-agent-status
description: 全エージェント（大臣・市民1-7・司祭）の稼働状態を一覧表示するスキル。tmux pane状態（稼働中/待機中/不在）とタスクYAML状態（task_id, status）と未読inbox数を統合表示。「稼働確認」「エージェント状態」「布陣確認」「agent status」で起動。
---
...
- **Pane=不在**: tmux paneが存在しない（departure未実行 or pane killed）。
