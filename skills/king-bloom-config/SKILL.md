...
# /king-bloom-config — Bloom Routing Wizard
...
3. 固定エージェント推奨モデル（Minister / Priest）
...
options:
  - label: "なし（Claude onlyで運用）"
    description: "Claude枠のみ。シンプル構成。市民はHaiku4.5が主力。"
  - label: "Plus ($20/月)"
    description: "gpt-5.3-codex利用可（Spark不可）。L4まで補完できる。"
  - label: "Pro ($200/月)"
    description: "Spark(1000 tok/s, Terminal-Bench 58.4%) + gpt-5.3(77.3%)利用可。市民7体の最強構成 (Recommended)"
```
...
| エージェント | 推奨モデル | 備考 |
|------------|-----------|------|
| Minister (大臣) | `claude-sonnet-4-6` | Opusは使えないのでSonnet |
| Priest (司祭) | `claude-sonnet-4-6` | 同上 |
...
> Claude Opusまで全モデル利用可。市民はHaiku(L1-L3)→Sonnet(L4-L5)→Opus(L6)で自動ルーティング。

### 固定エージェント

| エージェント | 推奨モデル | 備考 |
|------------|-----------|------|
| Minister (大臣) | `claude-sonnet-4-6` | L4-L5オーケストレーション。Opusは過剰。 |
| Priest (司祭) | `claude-opus-4-6` | L5-L6の深いQC・アーキテクチャ評価 |
...
> Claude サブスクなし → Minister/Priest も Codex モデル使用。L6 ギャップに注意。

| エージェント | 推奨モデル |
|------------|-----------|
| Minister (大臣) | `gpt-5.3-codex` |
| Priest (司祭) | `gpt-5.1-codex-max` |
...
capability_tiers:
  gpt-5.3-codex-spark:
    max_bloom: 3       # L1-L3: 1000+ tok/s — 市民7体でも余裕のスループット
...
| エージェント | 推奨モデル |
|------------|-----------|
| Minister (大臣) | `claude-sonnet-4-6` |
| Priest (司祭) | `claude-opus-4-6` |
...
| エージェント | 推奨モデル | 理由 |
|------------|-----------|------|
| Minister (大臣) | `claude-sonnet-4-6` | L4-L5オーケストレーション。SWE-bench 79.6% |
| Priest (司祭) | `claude-opus-4-6` | L5-L6深いQC。SWE-bench 80.8% |
...
cli:
  agents:
    minister:
      type: claude
      model: claude-sonnet-4-6     # ← Minister推奨モデルに変更
    priest:
      type: claude
      model: opus                  # ← Priest推奨モデルに変更
    citizen1:                     # ← 市民はcapability_tiersに従って自動ルーティング
