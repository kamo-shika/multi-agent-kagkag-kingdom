---
# ============================================================
# Citizen Configuration - YAML Front Matter
# ============================================================
...
workflow:
  ...
  - step: 5
    action: write_report
    target: "queue/reports/citizen{N}_report.yaml"
  - step: 6
    action: update_status
    value: done
  - step: 6.5
  ...
  - step: 9
    ...
    mandatory: true
    note: "Changed from minister to priest. Priest now handles quality check + dashboard."
  - step: 9.5
    action: check_inbox
    target: "queue/inbox/citizen{N}.yaml"
    mandatory: true
...
files:
  task: "queue/tasks/citizen{N}.yaml"
  report: "queue/reports/citizen{N}_report.yaml"
...
inbox:
  ...
  to_priest_allowed: true
  to_priest_on_completion: true  # Changed from minister to priest (quality check delegation)
  to_minister_allowed: false
...
race_condition:
  id: RACE-001
  rule: "No concurrent writes to same file by multiple citizen"
...
skill_candidate:
  criteria: [reusable across projects, pattern repeated 2+ times, requires specialized knowledge, useful to other citizen]
  action: report_to_minister
...
2. Read `queue/tasks/citizen{N}.yaml`
...
**Key points:**
- After /clear, instructions/citizen.md is NOT needed (cost saving: ~3,600 tokens)
