#!/bin/bash
# 🏰 multi-agent-kagkag-kingdom 出陣スクリプト（毎日の起動用）
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo ""
            echo "🏰 multi-agent-kagkag-kingdom 出陣スクリプト"
            echo ""
            ...
            echo "  -k, --kessen        決戦の陣（全市民をOpusで起動）"
            echo "                      未指定時は平時の陣（市民1-7=Sonnet, 司祭=Opus）"
            ...
            echo ""
            echo "例:"
            echo "  ./departure.sh              # 前回の状態を維持して出陣"
            ...
            echo "  ./departure.sh -k           # 決戦の陣（全市民Opus）"
            echo "  ./departure.sh -c -k         # クリーンスタート＋決戦の陣"
            ...
            echo ""
            echo "陣形:"
            echo "  平時の陣（デフォルト）: 市民1-7=Sonnet, 司祭=Opus"
            echo "  決戦の陣（--kessen）:   全市民=Opus, 司祭=Opus"
            echo ""
            echo "表示モード:"
...
CITIZEN_EOF
    ...
    echo -e "\033[1;33m  ┃\033[0m  \033[1;37m🏰 multi-agent-kagkag-kingdom\033[0m  〜 \033[1;36mKagKag Kingdomマルチエージェント統率システム\033[0m 〜           \033[1;33m┃\033[0m"
...
tmux kill-session -t king 2>/dev/null && log_info "  └─ king本陣、撤収完了" || log_info "  └─ king本陣は存在せず"
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    if [ "$KESSEN_MODE" = true ]; then
        ...
        log_info "  └─ 市民1-${_CITIZEN_COUNT}（決戦の陣）、召喚完了"
    else
        # 平時の陣: CLI Adapter経由（デフォルト: 全市民=Sonnet）
        for i in $(seq 1 "$_CITIZEN_COUNT"); do
            p=$((PANE_BASE + i))
            _ashi_cli_type="claude"
        ...
        log_info "  └─ 市民1-${_CITIZEN_COUNT}（平時の陣）、召喚完了"
    fi
    ...
    if [ "$KESSEN_MODE" = true ]; then
        ...
        log_success "✅ 平時の陣で出陣（大臣=Sonnet, 市民=Sonnet, 司祭=Opus）"
    fi
    echo ""
...
echo "  └──────────────────────────────────────────────────────────┘"
echo ""
echo "     【kingセッション】キングの本陣"
echo "     ┌─────────────────────────────┐"
echo "     │  Pane 0: キング (KING)      │  ← 総大将・プロジェクト統括"
echo "     └─────────────────────────────┘"
echo ""
echo "     【multiagentセッション】大臣・市民・司祭の陣（3x3 = 9ペイン）"
echo "     ┌─────────┬─────────┬─────────┐"
