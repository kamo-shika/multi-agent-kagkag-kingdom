...
市民がタスクを実行する中で、**再利用可能なパターンを自動的に発見**し、スキル候補として提案します。大臣が提案を `dashboard.md` に集約し、殿（あなた）が正式なスキルに昇格させるか判断します。
...
| エージェント | モデル | 思考モード | 役割 |
|-------------|--------|----------|------|
| キング | Opus | **有効（high）** | 殿の参謀。`--king-no-thinking` で中継専用モードに |
...
4. **割り込み防止**: キングが更新すると、殿の入力中に割り込む恐れあり
